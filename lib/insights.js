...
// Metadata-only loader (performance) — frontmatter only, cached for the build.
// The related-articles engine and cards only need { slug, kind, meta }, never
// the rendered insight body. Avoids running the Markdown pipeline per page.
let _insightMetaCache = null;

export function getAllInsightsMeta() {
  if (_insightMetaCache) return _insightMetaCache;
  ...
  const list = slugs
    .map((slug) => {
      ...
      return { slug, kind: 'insight', meta: data };
    })
    .filter(Boolean)
    .sort((a, b) => (b.meta.date || '').localeCompare(a.meta.date || ''));
  _insightMetaCache = list;
  return list;
}
