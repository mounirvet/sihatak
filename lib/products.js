...
function readProductFile(filename) {
  ...
  return {
    ...
    bundles: data.bundles || null,         // [{qty, price, label, badge?, best?}, ...] — quantity selector
    trust_chips: data.trust_chips || null, // [str, ...] — scannable trust chips near CTA
...
// Buyable = in stock and priced. Used to gate the Buy button.
//
// This USED to also require a `stripe_payment_link`, because each product had
// its own hosted Stripe checkout URL. With Snipcart the cart lives on our own
// domain and builds the order client-side, so no per-product link exists or is
// needed. The `stripe_payment_link` field is now dead data in the frontmatter —
// harmless, but it no longer gates anything.
export function isBuyable(product) {
  return product.in_stock && typeof product.price === "number" && product.price > 0;
}
